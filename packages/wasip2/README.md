...
- **Async mode** — Pass `async: true` to `WASIShim` config when using JSPI or asyncify. This enables async-compatible implementations (e.g., async filesystem descriptor handlers via `FsDescriptorHandler`) that return Promises from WASI calls, which are then suspended/resumed by the JSPI/asyncify runtime. JSPI supports multiple suspended stacks concurrently; asyncify is limited to one in-flight suspension per WASM instance (but each process is a separate instance, so pipelines work correctly).
- **Stream lifecycle (borrow ownership)** — `WASIShim` owns its stdio streams. `getImportObject()` exposes them as `borrow()` — non-ref-counted views whose `[Symbol.dispose]()` is a no-op, so JCO's resource-drop is harmless. `WASIShim[Symbol.dispose]()` drops the owned streams (ref 1->0 triggers `handler.drop()` for EOF/broken-pipe propagation). The handler is responsible for calling `shim[Symbol.dispose]()` in its `finally` block. `close()` remains as a force-drop utility but is not used in normal async pipeline flow.
