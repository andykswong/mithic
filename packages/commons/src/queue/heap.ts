...
export class BinaryHeap<T> implements Queue<T> {
  ...
  private heapifyDown(index: number): void {
    ...
    while (index < this.length) {
      ...
      if (leftIndex < this.length && this.compare(this.heap[leftIndex], this.heap[smallerChildIndex]) < 0) {
        smallerChildIndex = leftIndex;
      }
      if (rightIndex < this.length && this.compare(this.heap[rightIndex], this.heap[smallerChildIndex]) < 0) {
