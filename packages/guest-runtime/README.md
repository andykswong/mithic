...
- **`createGuest`** — top-level factory. Consumes the `control` port, the
  ...
  `args`, `env`, `cwd`, `stdin`, `stdout`, `stderr`, `syscall`, `onSignal`,
  `onDomEvent`, and `exit`. It multiplexes the control port, routing syscall
  responses to the `SyscallClient` and kernel events (`signal`, `dom/event`) to
