 ...
 */
import { DefaultGuestLauncher } from '@mithic/kernel';
 ...
 * invokes it with the kernel's boot object (control + stdio ports). Any OTHER
 * code (a real inline-source string or module URL — e.g. the notebook's inline
 * image-viewer GUI guest) is delegated to a {@link DefaultGuestLauncher}, which
 * spawns it via the runtime (so `display:inline` GUI processes still mount).
 */
export class InProcessCommandLauncher implements GuestLauncher {
  readonly #registry: Map<string, () => Promise<GuestDefault>>;
  readonly #fallback = new DefaultGuestLauncher();
  ...
  async launch(runtime: Runtime, ctx: LaunchContext): Promise<ProcessHandle> {
    ...
    if (!href.startsWith(COMMAND_SCHEME)) {
      // Not a registered command — let the default launcher run it on the runtime.
      return this.#fallback.launch(runtime, ctx);
    }
  ...
  kill(runtime: Runtime, handle: ProcessHandle, signal: Signal): void {
    // Command guests run in-process and exit on their own; delegate kill for
    // runtime-backed (fallback-launched) processes like the GUI image-viewer.
    this.#fallback.kill?.(runtime, handle, signal);
  }
}
