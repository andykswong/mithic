...
import { RemoteKeyValueStore } from './index.ts';
import { KVStoreMessage, KVStoreOp } from './codec.ts';

const BUCKET = 'bucket1';

describe('RemoteKeyValueStore', () => {
  ...
  describe('open', () => {
    it('should send open request', async () => {
      ...
      assert.strictEqual(client.open(BUCKET), bucketId);
      await delay(100);
  ...
  describe('exists', () => {
    it('should send exist request', async () => {
      ...
      assert.strictEqual(client.exists(BUCKET, key), true);
      await delay(100);
    ...
    it('throws if trying to access a non-opened store', () => {
      ...
      assert.throws(() => client.exists('testtest', 'test'), new StoreError({ tag: StoreErrorType.NoSuchStore }));
    });
  });
  ...
  describe('listKeys', () => {
    it('should send keys request', async () => {
      ...
      assert.deepStrictEqual(client.listKeys(BUCKET, selector, cursor2), { keys: [key], cursor })
      await delay(100);
  ...
  describe('getMany', () => {
    it('should send get request', async () => {
      ...
      assert.deepStrictEqual(client.getMany(BUCKET, [key2, key]), [null, value])
      await delay(100);
  ...
  describe('increment', () => {
    it('should send incr request', async () => {
      ...
      assert.deepStrictEqual(client.increment(BUCKET, key, delta), result);
      await delay(100);
  ...
  describe('compareAndSwap', () => {
    it('should send CAS request', async () => {
      ...
      host.send({ op: KVStoreOp.Response, seq, success: true });
      assert.strictEqual(client.compareAndSwap(BUCKET, key, oldValue, newValue), true);
      await delay(100);
