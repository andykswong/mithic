...
describe(IDBKeyValueStore.name, () => {
  ...
  it('should have correct string tag', () => {
    expect(store.toString()).toBe(`[object ${IDBKeyValueStore.name}]`);
  });
  ...
  describe('getMany', () => {
    ...
    it('should get data from store', async () => {
      expect(await store.getMany(bucket, [key1, 'unknown', key2])).toEqual([value1, null, value2]);
    });
  ...
  describe('updateMany', () => {
    ...
    it('should set or deleta data in local storage', async () => {
      ...
      expect(await store.getMany(bucket, [key1, key2, key3])).toEqual([null, value2, value3]);
    });
  ...
  describe('increment', () => {
    ...
    it('should increment stored number', async () => {
      ...
      expect(await store.getMany(bucket, [key])).toEqual([encode(0x16an)]);
    });
