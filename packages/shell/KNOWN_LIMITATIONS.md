...
| `readonly` re-assignment rejection | `readonly RO=1; RO=2` is rejected: the old value is kept and a `shell: RO: readonly variable` diagnostic is written. The `readonly` builtin records names on the executor's readonly set (after applying its own `NAME=val`, so the first assignment succeeds); `applyAssignment` checks it. Rejection covers bare reassignment, `unset RO`, `export RO=…`/`declare RO=…`, and a write THROUGH a `declare -n` nameref to a readonly target (the readonly check runs after the nameref rewrite). In posix mode the bare-reassignment rejection throws `PosixSpecialBuiltinError` (the assignment-error fatal path of POSIX 2.8.1 — see above); outside posix mode it reports + continues with status 1. *(Still NOT enforced — minor, untested: a `for X in …` loop variable, a `getopts` var, and a `X=2 cmd` prefix-overlay can overwrite a readonly. `${ref:=x}` default-ASSIGN through a nameref writes the literal ref name. Documented follow-ups.)* | **done** |
