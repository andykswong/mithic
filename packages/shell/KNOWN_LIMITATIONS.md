...
| **binary through in-process pipes / redirects** | A spawned guest's stdout/stderr now reach the terminal (and a captured last pipeline stage) BYTE-EXACT — the shell forwards raw bytes to its output sink with no UTF-8 round-trip, so `cat <binary>` is faithful. Still NOT byte-safe: an in-process pipeline stage capture (`captured += s`) and a file redirect (`> file`, which writes through a string `fsWrite`) round-trip through UTF-8. Builtin stdin (`cat`/`read`/`mapfile`) is also still a materialized string. | **pending** |
